    ...
    <div class="section-content">
        ...
        <div id="inlineServiceForms" class="mt-5">
            <div id="liftingExaminationFormSection" style="display:none;">
                @include('inspections.edit-wizard-sections.lifting-examination')
            </div>
            <div id="mpiServiceFormSection" style="display:none;">
                @include('inspections.edit-wizard-sections.mpi-service')
            </div>
            <div id="loadTestFormSection" style="display:none;">
                @include('inspections.edit-wizard-sections.load-test')
            </div>
            <div id="thoroughExaminationFormSection" style="display:none;">
                @include('inspections.edit-wizard-sections.thorough-examination')
            </div>
            <div id="visualFormSection" style="display:none;">
                @include('inspections.edit-wizard-sections.visual')
            </div>
            <div id="otherServicesFormSection" style="display:none;">
                @include('inspections.edit-wizard-sections.other-services')
            </div>
        </div>
    </div>
</section>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Build preselection list based on existing data and relations
    @php
        $servicesPerformed = $inspection->services_performed ?? [];
        if (is_string($servicesPerformed)) {
            $decoded = json_decode($servicesPerformed, true);
            $servicesPerformed = is_array($decoded) ? $decoded : [];
        }

        // Start with services_performed, then add relation-backed services
        $preselectServices = $servicesPerformed ?: [];
        if ($inspection->liftingExamination) { $preselectServices[] = 'Lifting Examination'; }
        if ($inspection->loadTest) { $preselectServices[] = 'Load Test'; }
        if ($inspection->mpiInspection) { $preselectServices[] = 'MPI Service'; }
        if ($inspection->otherTest) { $preselectServices[] = 'Other Tests'; }
        // De-duplicate
        $preselectServices = array_values(array_unique($preselectServices));
    @endphp
    const existingServices = @json($preselectServices);
    console.log('DEBUG: Preselect services (combined):', existingServices);
