                ...
                <div class="inspection-question mb-4">
                    <div class="card border-primary">
                        ...
                        <div class="card-body">
                                    ...
                                    <div class="d-flex gap-3">
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="first_examination" id="firstExamYes" value="yes" 
                                                   {{ old('first_examination', $inspection?->liftingExamination?->first_examination ?? '') == 'yes' ? 'checked' : '' }}
                                                   onchange="handleFirstExamination(this)">
                                        ...
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="first_examination" id="firstExamNo" value="no" 
                                                   {{ old('first_examination', $inspection?->liftingExamination?->first_examination ?? '') == 'no' ? 'checked' : '' }}
                                                   onchange="handleFirstExamination(this)">
                            ...
                            <div id="firstExamFollowUp" class="mt-4" style="display: {{ old('first_examination', $inspection?->liftingExamination?->first_examination ?? '') == 'yes' ? 'block' : 'none' }};">
                                <div class="row">
                                    <!-- If Yes Section -->
                                    <div class="col-md-6">
                                        <div class="border p-3 rounded bg-success bg-opacity-10">
                                            ...
                                            <div class="d-flex gap-3 mb-3">
                                                <div class="form-check">
                                                    <input class="form-check-input" type="radio" name="equipment_installed_correctly" value="yes"
                                                           {{ old('equipment_installed_correctly', $inspection?->liftingExamination?->equipment_installed_correctly ?? '') == 'yes' ? 'checked' : '' }}>
                                                    <label class="form-check-label">Yes</label>
                                                </div>
                                                <div class="form-check">
                                                    <input class="form-check-input" type="radio" name="equipment_installed_correctly" value="no"
                                                           {{ old('equipment_installed_correctly', $inspection?->liftingExamination?->equipment_installed_correctly ?? '') == 'no' ? 'checked' : '' }}>
                        ...
                        <div class="card-body">
                            <div class="row align-items-center">
                                <div class="col-md-2">
                                    <div class="d-flex gap-3">
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="safe_to_operate" value="yes"
                                                   {{ old('safe_to_operate', $inspection?->liftingExamination?->safe_to_operate ?? '') == 'yes' ? 'checked' : '' }}>
                                            <label class="form-check-label fw-semibold text-success">
                                                Yes
                                            </label>
                                        </div>
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="safe_to_operate" value="no"
                                                   {{ old('safe_to_operate', $inspection?->liftingExamination?->safe_to_operate ?? '') == 'no' ? 'checked' : '' }}>
                ...
                <div class="inspection-question mb-4">
                    <div class="card border-danger">
                        ...
                        <div class="card-body">
                                    ...
                                    <div class="d-flex gap-3">
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="equipment_defect" id="defectYes" value="yes" 
                                                   {{ old('equipment_defect', $inspection?->liftingExamination?->equipment_defect ?? '') == 'yes' ? 'checked' : '' }}
                                                   onchange="handleEquipmentDefect(this)">
                                        ...
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="equipment_defect" id="defectNo" value="no" 
                                                   {{ old('equipment_defect', $inspection?->liftingExamination?->equipment_defect ?? '') == 'no' ? 'checked' : '' }}
                                                   onchange="handleEquipmentDefect(this)">
                            ...
                            <!-- Conditional Defect Details (always rendered; shown when 'Yes' is selected) -->
                            <div id="defectDetails" class="mt-4" style="display: none;">
...
console.log('📦 liftingExamination relation data:', @json($inspection?->liftingExamination));
if (@json($inspection?->liftingExamination) === null) {
    console.warn('⚠️ No liftingExamination record found for this inspection');
} else {
    const le = @json($inspection?->liftingExamination);
    console.table({
        first_examination: le.first_examination,
        first_examination_notes: le.first_examination_notes,
        equipment_installed_correctly: le.equipment_installed_correctly,
        equipment_installation_details: le.equipment_installation_details,
        examination_carried_out: le.examination_carried_out,
        safe_to_operate: le.safe_to_operate,
        safe_to_operate_notes: le.safe_to_operate_notes,
        equipment_defect: le.equipment_defect,
        equipment_defect_notes: le.equipment_defect_notes,
        defect_description: le.defect_description,
        existing_danger: le.existing_danger,
        potential_danger: le.potential_danger,
        defect_timeline: le.defect_timeline,
        repair_details: le.repair_details,
        test_details: le.test_details
    });
}
...
window.handleEquipmentDefect = function(element) {
    ...
    // Determine selected value robustly
    const selected = element?.value ?? (document.querySelector('input[name="equipment_defect"]:checked')?.value || null);
    const isYes = selected === 'yes';

    if (defectDetailsSection) {
        defectDetailsSection.style.display = isYes ? 'block' : 'none';
        defectDetailsSection.style.visibility = isYes ? 'visible' : '';
        defectDetailsSection.style.opacity = isYes ? '1' : '';
        defectDetailsSection.classList.toggle('fade-in', isYes);
        defectDetailsSection.classList.toggle('d-none', !isYes);
        console.log(isYes ? '✅ Showing Question C defect details' : '❌ Hiding Question C defect details');
    }
};
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('🧭 Selected defect value at init:', selectedDefect?.value);
    // Always initialize visibility based on current selection
