...
class InspectionController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'lead_inspector_name' => 'nullable|string|max:255', // Changed from inspector_name
        ...
        try {
            ...
            if ($request->inspection_id) {
                ...
                $safeOptionalFields = [
                    'area_of_examination', 'services', 'standards', 'job_ref', 
                    'weather_conditions', 'inspector_comments', 'recommendations', 
                ...
                $createData = [
                    ...
                    // Required fields that need defaults
                    'applicable_standard' => $request->filled('standards') ? $request->standards : 'General Standard',
                    'inspection_class' => $request->filled('inspection_class') ? $request->inspection_class : 'General',
                    'overall_result' => 'pass',
                ];
                ...
                $safeOptionalFields = [
                    ...
                    'manufacture_year', 'capacity', 'capacity_unit',
                    'certification_body', 'previous_certificate_number', 
                    'last_inspection_date', 'next_inspection_due', 'general_notes',
                    // Additional inspection fields
                    'temperature', 'humidity', 'limitations', 'inspector_signature'
                ];
    ...
    /**
     * Save inspection as draft (auto-save functionality)
     */
    public function saveDraft(Request $request)
    {
        try {
            ...
            // Create data with required fields and defaults
            $createData = [
                'inspection_number' => Inspection::generateInspectionNumber(),
                'status' => 'completed',
                'report_date' => now(),
                'project_name' => $request->filled('project_name') ? $request->project_name : 'Draft Project',
                'lead_inspector_name' => $request->filled('lead_inspector_name') ? $request->lead_inspector_name : 'TBD',
            ...
            $allFields = [
                'client_name', 'location', 'inspection_date',
                ...
                'next_inspection_due', 'lead_inspector_certification', 'general_notes', 'inspection_images'
            ];

            foreach ($allFields as $field) {
                if ($request->filled($field)) {
                    if ($field === 'inspection_images') {
                        $createData[$field] = json_encode($request->$field);
                    } else {
                        $createData[$field] = $request->$field;
                    }
                }
            ...
            // Create the main inspection record as auto-save
            $inspection = Inspection::create($createData);
